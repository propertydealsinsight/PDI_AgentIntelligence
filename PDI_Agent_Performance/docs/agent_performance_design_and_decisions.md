...
| T4 | **Procedure-era cleanup — DONE 2026-07-13** (pending the user executing the DROP statements): drop `event_generate_agent_performance`, `p_generate_agent_performance`, `p_generate_agent_performance_corrected`, and its 87 `pdi_procedure_logs` rows. Repo copy of the corrected procedure deleted (git history retains it). `pdi_agent_performance_legacy_DND` deliberately kept as the last pre-cutover snapshot — drop after a few clean weekly cycles (~end July 2026). `event_generate_agent_master` is NOT part of this and stays enabled (it builds the master table the pipeline depends on). | Execute the SQL block, then only legacy_DND remains |
